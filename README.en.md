...
> Current baseline: `@deepseek-ai/dsh@0.1.7-alpha.2`. DeepSeek Harness is still a pre-release. Re-run the build and smoke tests before every version upgrade.

`0.1.7-alpha.2` maps directly to the official [`dsh-v0.1.7-alpha.2`](https://github.com/deepseek-ai/deepseek-harness/releases/tag/dsh-v0.1.7-alpha.2) Release and [`@deepseek-ai/dsh@0.1.7-alpha.2`](https://www.npmjs.com/package/@deepseek-ai/dsh/v/0.1.7-alpha.2) in the npm Registry; it is not a project-defined version. This project packages the installable npm distribution instead of building source, and intentionally does not publish a drifting Docker `latest` tag.

The explicit npm version is available upstream; registry replicas and dist-tags may briefly lag, so this image always pins the complete version. Upstream `0.1.7-alpha.2` stabilizes conversation scrolling, history pagination, successive background-task wakeups, and Web reconnection after a restart, while improving code blocks, Excel previews, model discovery, and first-install registry selection.
...
The default immutable image revision is [`runzhliu/deepseek-harness:0.1.7-alpha.2-r1`](https://hub.docker.com/r/runzhliu/deepseek-harness). The same multi-platform artifact is also published to GitHub Container Registry as [`ghcr.io/runzhliu/deepseek-harness:0.1.7-alpha.2-r1`](https://github.com/users/runzhliu/packages/container/package/deepseek-harness). `r1` attaches the matching official Playwright MCP Browser Use provider to the visible Chromium and retains versioned noVNC asset paths so an upgrade cannot mix incompatible cached ES modules. Compose keeps the build definition for reproducibility and review; run `docker compose build --pull` before startup when you explicitly want a local build.
...
The variant has the unambiguous `runzhliu/deepseek-harness:0.1.7-alpha.2-r1-market.1` tag and pins `dshmarket@1.38.1`; it does not replace the default DSH tag or `latest`. It is an optional community integration, not a DeepSeek component, and neither DeepSeek nor this project audits or endorses catalog entries.
...
Helm continues to default to the official-DSH image; it uses the market variant only when you explicitly pass `--set image.tag=0.1.7-alpha.2-r1-market.1`.
...
```bash
docker build -t runzhliu/deepseek-harness:0.1.7-alpha.2-r1 .
```
...
docker run --rm \
  ...
  runzhliu/deepseek-harness:0.1.7-alpha.2-r1
```
...
helm upgrade --install deepseek-harness charts/deepseek-harness \
  ...
  --set image.tag=0.1.7-alpha.2-r1
```
...
```bash
docker build \
  --build-arg DSH_VERSION=0.1.7-alpha.2 \
  --build-arg IMAGE_VERSION=0.1.7-alpha.2-r1 \
  -t runzhliu/deepseek-harness:0.1.7-alpha.2-r1 .
```

Compose keeps the upstream version and immutable container revision separate:

```bash
DSH_VERSION=0.1.7-alpha.2 DSH_IMAGE_VERSION=0.1.7-alpha.2-r1 docker compose build --pull
```
...
```bash
docker run --rm runzhliu/deepseek-harness:0.1.7-alpha.2-r1 --version

docker run --rm --entrypoint dsh runzhliu/deepseek-harness:0.1.7-alpha.2-r1 \

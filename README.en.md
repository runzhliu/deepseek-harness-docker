...
> Current baseline: `@deepseek-ai/dsh@0.1.3-alpha.2`. DeepSeek Harness is still a pre-release. Re-run the build and smoke tests before every version upgrade.

`0.1.3-alpha.2` maps directly to the official [`dsh-v0.1.3-alpha.2`](https://github.com/deepseek-ai/deepseek-harness/releases/tag/dsh-v0.1.3-alpha.2) Release and [`@deepseek-ai/dsh@0.1.3-alpha.2`](https://www.npmjs.com/package/@deepseek-ai/dsh/v/0.1.3-alpha.2) in the npm Registry; it is not a project-defined version. This project packages the installable npm distribution instead of building source, and intentionally does not publish a drifting Docker `latest` tag.

This version is currently published under npm's `alpha` dist-tag while `latest` remains `0.1.2-rc.1`; the image installs the explicit full version so dist-tag movement cannot change a build. Upstream `0.1.3-alpha.2` improves responsiveness and memory use when resuming long sessions and includes Web reconnect fixes, new-model support, and Session v2 migration.

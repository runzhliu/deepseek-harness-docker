...
> Current baseline: `@deepseek-ai/dsh@0.1.6-alpha.2`. DeepSeek Harness is still a pre-release. Re-run the build and smoke tests before every version upgrade.

`0.1.6-alpha.2` maps directly to the official [`dsh-v0.1.6-alpha.2`](https://github.com/deepseek-ai/deepseek-harness/releases/tag/dsh-v0.1.6-alpha.2) Release and [`@deepseek-ai/dsh@0.1.6-alpha.2`](https://www.npmjs.com/package/@deepseek-ai/dsh/v/0.1.6-alpha.2) in the npm Registry; it is not a project-defined version. This project packages the installable npm distribution instead of building source, and intentionally does not publish a drifting Docker `latest` tag.

The explicit npm version is available upstream; registry replicas and dist-tags may briefly lag, so this image always pins the complete version. Upstream `0.1.6-alpha.2` adds a Plugins page, end-of-turn file-change review, Office previews, and sidebar views for URLs, Subagent conversations, and submitted plans. It also improves workspace grouping, persistent sidebar layouts, attachment presentation, startup diagnostics, and model input-type detection.
...
> **Compatibility warning:** the `0.1.6` series makes DeepSeek Messages the default protocol, disables Ralph by default, removes the built-in E2B backends, and renames the PTC and workflow services. `alpha.2` additionally resolves and unloads plugin dependencies at runtime, replaces the former Creator-mode Cordis tools with persistent Plugin Manager installation, and changes Client Session APIs and slots. Custom providers, plugins, and Cordis patches must be checked against the [upstream Release](https://github.com/deepseek-ai/deepseek-harness/releases/tag/dsh-v0.1.6-alpha.2).

...
> Current baseline: `@deepseek-ai/dsh@0.1.6-alpha.1`. DeepSeek Harness is still a pre-release. Re-run the build and smoke tests before every version upgrade.

`0.1.6-alpha.1` maps directly to the official [`dsh-v0.1.6-alpha.1`](https://github.com/deepseek-ai/deepseek-harness/releases/tag/dsh-v0.1.6-alpha.1) Release and [`@deepseek-ai/dsh@0.1.6-alpha.1`](https://www.npmjs.com/package/@deepseek-ai/dsh/v/0.1.6-alpha.1) in the npm Registry; it is not a project-defined version. This project packages the installable npm distribution instead of building source, and intentionally does not publish a drifting Docker `latest` tag.

The explicit npm version is available upstream; registry replicas and dist-tags may briefly lag, so this image always pins the complete version. Upstream `0.1.6-alpha.1` adds a Web sidebar terminal, archived-session recovery, MCP Resources, resumable Headless sessions, and experimental Browser Use, Computer Use, and Auto review, together with file-preview, trajectory, persistent-Bash, and long-task improvements.
...
> **Compatibility warning:** `0.1.6-alpha.1` makes DeepSeek Messages the default protocol, disables Ralph by default, removes the built-in E2B backends, and renames the PTC and workflow services. Custom providers, plugins, and Cordis patches that depend on the old APIs or service names must follow the [upstream Release](https://github.com/deepseek-ai/deepseek-harness/releases/tag/dsh-v0.1.6-alpha.1).
...
The variant has the unambiguous `runzhliu/deepseek-harness:0.1.6-alpha.1-r1-market.1` tag and pins `dshmarket@1.38.1`; it does not replace the default DSH tag or `latest`. It is an optional community integration, not a DeepSeek component, and neither DeepSeek nor this project audits or endorses catalog entries.
...
Helm continues to default to the official-DSH image; it uses the market variant only when you explicitly pass `--set image.tag=0.1.6-alpha.1-r1-market.1`.
...
```bash
docker build -t runzhliu/deepseek-harness:0.1.6-alpha.1-r1 .
```
...
docker run --rm \
  ...
  runzhliu/deepseek-harness:0.1.6-alpha.1-r1
```
...
helm upgrade --install deepseek-harness charts/deepseek-harness \
  ...
  --set image.tag=0.1.6-alpha.1-r1
```
...
docker build \
  ...
  --build-arg IMAGE_VERSION=0.1.6-alpha.1-r1 \
  -t runzhliu/deepseek-harness:0.1.6-alpha.1-r1 .
```

Compose keeps the upstream version and immutable container revision separate:

```bash
DSH_VERSION=0.1.6-alpha.1 DSH_IMAGE_VERSION=0.1.6-alpha.1-r1 docker compose build --pull
```
...
```bash
docker run --rm runzhliu/deepseek-harness:0.1.6-alpha.1-r1 --version

docker run --rm --entrypoint dsh runzhliu/deepseek-harness:0.1.6-alpha.1-r1 \

...
> Current baseline: `@deepseek-ai/dsh@0.1.5-alpha.1`. DeepSeek Harness is still a pre-release. Re-run the build and smoke tests before every version upgrade.

`0.1.5-alpha.1` maps directly to the official [`dsh-v0.1.5-alpha.1`](https://github.com/deepseek-ai/deepseek-harness/releases/tag/dsh-v0.1.5-alpha.1) Release and [`@deepseek-ai/dsh@0.1.5-alpha.1`](https://www.npmjs.com/package/@deepseek-ai/dsh/v/0.1.5-alpha.1) in the npm Registry; it is not a project-defined version. This project packages the installable npm distribution instead of building source, and intentionally does not publish a drifting Docker `latest` tag.

This version is currently published under npm's `alpha` dist-tag while `latest` remains `0.1.2-rc.1`; the image installs the explicit full version so dist-tag movement cannot change a build. Upstream `0.1.5-alpha.1` adds dynamic system prompts and an experimental right Sidebar, and fixes send queuing, paused goals, local-image rendering, and project-root error handling.

> **Upgrade warning:** this release migrates supported historical sessions to Session Format V3 while preserving the original logs. Migrated sessions cannot be read by older DSH releases, and custom log readers must be updated for V3. Back up `dsh-home` before upgrading.
...
The default immutable image revision is [`runzhliu/deepseek-harness:0.1.5-alpha.1-r1`](https://hub.docker.com/r/runzhliu/deepseek-harness). The same multi-platform artifact is also published to GitHub Container Registry as [`ghcr.io/runzhliu/deepseek-harness:0.1.5-alpha.1-r1`](https://github.com/users/runzhliu/packages/container/package/deepseek-harness). `r1` is the first container revision for this upstream version. The image continues to serve noVNC assets from a versioned path so an upgrade cannot make a browser mix incompatible cached ES modules. Compose retains the `build` definition so the image remains reproducible and reviewable; run `docker compose build --pull` before startup when you explicitly want a local build.
...
The default image keeps Debian Chromium for Debian's security-update and distribution supply chain. Select the separate [`runzhliu/deepseek-harness:0.1.5-alpha.1-r1-ungoogled.1`](https://hub.docker.com/r/runzhliu/deepseek-harness/tags) variant only when idle browser egress must be minimized:
...
The variant has the unambiguous `runzhliu/deepseek-harness:0.1.5-alpha.1-r1-market.1` tag and pins `dshmarket@1.38.1`; it does not replace the default DSH tag or `latest`. It is an optional community integration, not a DeepSeek component, and neither DeepSeek nor this project audits or endorses catalog entries.
...
Helm continues to default to the official-DSH image; it uses the market variant only when you explicitly pass `--set image.tag=0.1.5-alpha.1-r1-market.1`.
...
```bash
docker run --rm runzhliu/deepseek-harness:0.1.5-alpha.1-r1 --version

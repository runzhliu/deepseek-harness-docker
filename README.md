...
> 当前基线：`@deepseek-ai/dsh@0.1.6-alpha.1`。DeepSeek Harness 仍处于预发布阶段；升级前应重新完成本文的构建和 Smoke Test。

`0.1.6-alpha.1` 直接对应官方 [`dsh-v0.1.6-alpha.1`](https://github.com/deepseek-ai/deepseek-harness/releases/tag/dsh-v0.1.6-alpha.1) Release 与 npm Registry 的 [`@deepseek-ai/dsh@0.1.6-alpha.1`](https://www.npmjs.com/package/@deepseek-ai/dsh/v/0.1.6-alpha.1)，并非本项目自定义版本。本项目封装 npm 成品而不从源码构建，因此以可安装的官方发行物为基线，并故意不发布漂移的 Docker `latest` 标签。

上游已发布可安装的显式 npm 版本；Registry 镜像与 dist-tag 的更新可能短暂滞后，本项目始终固定完整版本，避免 dist-tag 漂移。`0.1.6-alpha.1` 新增 Web 侧边栏终端、归档会话恢复、MCP Resources、可续接的 Headless 会话，以及实验性的 Browser Use、Computer Use 和 Auto review；同时改进文件预览、轨迹查看、持久 Bash 与长任务体验。
...
> **兼容性提醒：** `0.1.6-alpha.1` 默认改用 DeepSeek Messages 协议，默认关闭 Ralph，移除内置 E2B 后端，并重命名 PTC 与工作流服务；自定义 Provider、插件或 Cordis patch 如依赖旧 API/服务名，需要按[官方 Release](https://github.com/deepseek-ai/deepseek-harness/releases/tag/dsh-v0.1.6-alpha.1)同步调整。
...
默认镜像继续使用 Debian Chromium，以保留 Debian 安全更新与发行版供应链。对浏览器空闲后台连接有严格要求时，可显式选择独立的 [`runzhliu/deepseek-harness:0.1.6-alpha.1-r1-ungoogled.1`](https://hub.docker.com/r/runzhliu/deepseek-harness/tags) 变体：
...
该变体使用明确区分的 `runzhliu/deepseek-harness:0.1.6-alpha.1-r1-market.1` 标签，固定 `dshmarket@1.38.1`，不会替换默认 DSH 标签或 `latest`。它属于社区可选集成，不是 DeepSeek 官方组件，也不代表本项目对市场条目的审核或背书。
...
Helm 仍默认官方 DSH 镜像；只有明确设置 `--set image.tag=0.1.6-alpha.1-r1-market.1` 时才使用市场变体。
...
```bash
docker run --rm --entrypoint dsh runzhliu/deepseek-harness:0.1.6-alpha.1-r1 --version

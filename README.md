...
> 当前基线：`@deepseek-ai/dsh@0.1.5-rc.2`。DeepSeek Harness 仍处于预发布阶段；升级前应重新完成本文的构建和 Smoke Test。

`0.1.5-rc.2` 直接对应官方 [`dsh-v0.1.5-rc.2`](https://github.com/deepseek-ai/deepseek-harness/releases/tag/dsh-v0.1.5-rc.2) Release 与 npm Registry 的 [`@deepseek-ai/dsh@0.1.5-rc.2`](https://www.npmjs.com/package/@deepseek-ai/dsh/v/0.1.5-rc.2)，并非本项目自定义版本。本项目封装 npm 成品而不从源码构建，因此以可安装的官方发行物为基线，并故意不发布漂移的 Docker `latest` 标签。

上游已发布可安装的显式 npm 版本；Registry 镜像与 `latest`/`next` dist-tag 的更新可能短暂滞后，本项目始终固定完整版本，避免 dist-tag 漂移。`0.1.5-rc.2` 优化点赞和点踩的确认流程，提交失败时保留已填写反馈，并调整交付文件卡片、对话间距和代码文件图标。

> **升级提醒：** `0.1.5` 系列使用 Session Format V3；从旧格式恢复受支持的历史会话时会生成 V3 日志并保留原始日志。迁移后的会话不能由旧版 DSH 读取。从 `0.1.5-alpha.1` 或更新版本升级仍为 V3，不会重复迁移；升级前仍建议备份 `dsh-home`，自定义日志读取器也需要适配 V3。
...
默认镜像修订版为 Docker Hub 上的 [`runzhliu/deepseek-harness:0.1.5-rc.2-r1`](https://hub.docker.com/r/runzhliu/deepseek-harness)，同一份多架构制品也会发布到 GitHub Container Registry：[`ghcr.io/runzhliu/deepseek-harness:0.1.5-rc.2-r1`](https://github.com/users/runzhliu/packages/container/package/deepseek-harness)。`r1` 是该上游版本的首个容器修订号；镜像继续使用带版本的 noVNC 静态资源路径，避免升级后浏览器缓存混用不兼容的 ES Module。Compose 同时保留 `build` 配置，方便审查并从本目录复现镜像；如需本地构建，执行 `docker compose build --pull` 后再启动。
...
默认镜像继续使用 Debian Chromium，以保留 Debian 安全更新与发行版供应链。对浏览器空闲后台连接有严格要求时，可显式选择独立的 [`runzhliu/deepseek-harness:0.1.5-rc.2-r1-ungoogled.1`](https://hub.docker.com/r/runzhliu/deepseek-harness/tags) 变体：
...
该变体使用明确区分的 `runzhliu/deepseek-harness:0.1.5-rc.2-r1-market.1` 标签，固定 `dshmarket@1.38.1`，不会替换默认 DSH 标签或 `latest`。它属于社区可选集成，不是 DeepSeek 官方组件，也不代表本项目对市场条目的审核或背书。
...
Helm 仍默认官方 DSH 镜像；只有明确设置 `--set image.tag=0.1.5-rc.2-r1-market.1` 时才使用市场变体。
...
```bash
docker run --rm --entrypoint dsh runzhliu/deepseek-harness:0.1.5-rc.2-r1 --version

...
> 当前基线：`@deepseek-ai/dsh@0.1.6-alpha.2`。DeepSeek Harness 仍处于预发布阶段；升级前应重新完成本文的构建和 Smoke Test。

`0.1.6-alpha.2` 直接对应官方 [`dsh-v0.1.6-alpha.2`](https://github.com/deepseek-ai/deepseek-harness/releases/tag/dsh-v0.1.6-alpha.2) Release 与 npm Registry 的 [`@deepseek-ai/dsh@0.1.6-alpha.2`](https://www.npmjs.com/package/@deepseek-ai/dsh/v/0.1.6-alpha.2)，并非本项目自定义版本。本项目封装 npm 成品而不从源码构建，因此以可安装的官方发行物为基线，并故意不发布漂移的 Docker `latest` 标签。

上游已发布可安装的显式 npm 版本；Registry 镜像与 dist-tag 的更新可能短暂滞后，本项目始终固定完整版本，避免 dist-tag 漂移。`0.1.6-alpha.2` 新增插件管理页、回合文件改动审阅、Office 文件预览，以及 URL、Subagent 会话和提交计划的侧边栏预览；同时改进工作区分组、侧边栏持久化、附件展示、启动诊断与模型输入类型识别。
...
> **兼容性提醒：** `0.1.6` 系列默认改用 DeepSeek Messages 协议，默认关闭 Ralph，移除内置 E2B 后端，并重命名 PTC 与工作流服务。`alpha.2` 进一步改为运行时解析和卸载插件依赖，创造模式也改由 Plugin Manager 安装持久化插件，Client Session API/slot 同时有调整；自定义 Provider、插件或 Cordis patch 需要按[官方 Release](https://github.com/deepseek-ai/deepseek-harness/releases/tag/dsh-v0.1.6-alpha.2)同步检查。

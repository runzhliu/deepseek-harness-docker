...
默认镜像修订版为 Docker Hub 上的 [`runzhliu/deepseek-harness:0.1.7-alpha.2-r1`](https://hub.docker.com/r/runzhliu/deepseek-harness)，同一份多架构制品也会发布到 GitHub Container Registry：[`ghcr.io/runzhliu/deepseek-harness:0.1.7-alpha.2-r1`](https://github.com/users/runzhliu/packages/container/package/deepseek-harness)。`r1` 将同版本官方 Playwright MCP Browser Use attach 到可视 Chromium，并继续使用带版本的 noVNC 静态资源路径，避免升级后浏览器缓存混用不兼容的 ES Module。Compose 同时保留 `build` 配置，方便审查并从本目录复现镜像；如需本地构建，执行 `docker compose build --pull` 后再启动。
...
默认镜像继续使用 Debian Chromium，以保留 Debian 安全更新与发行版供应链。对浏览器空闲后台连接有严格要求时，可显式选择独立的 [`runzhliu/deepseek-harness:0.1.7-alpha.2-r1-ungoogled.1`](https://hub.docker.com/r/runzhliu/deepseek-harness/tags) 变体：
...
该变体使用明确区分的 `runzhliu/deepseek-harness:0.1.7-alpha.2-r1-market.1` 标签，固定 `dshmarket@1.38.1`，不会替换默认 DSH 标签或 `latest`。它属于社区可选集成，不是 DeepSeek 官方组件，也不代表本项目对市场条目的审核或背书。
...
Helm 仍默认官方 DSH 镜像；只有明确设置 `--set image.tag=0.1.7-alpha.2-r1-market.1` 时才使用市场变体。
...
```bash
docker run --rm --entrypoint dsh runzhliu/deepseek-harness:0.1.7-alpha.2-r1 --version
